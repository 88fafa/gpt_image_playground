...
Before making the first request, ask the user for exactly these two values:

1. `Base URL`, including `/v1`, for example `https://image.example.com/v1`.
2. `API Key` for that API.

Keep the key in an environment variable or an in-memory secret for the current session. Never place it in a URL, source file, command history, log, screenshot, generated skill file, or commit. Do not ask for the key again when it is already available in the current session.
...
Use a Responses input array. Convert local reference files to data URLs or another format accepted by the gateway, and include one `input_image` item for each reference image:
...
- Always send `size`, such as `1024x1024`, `768x1024`, or `1024x768`.
- Put the selected ratio in the final sentence of `input` or `prompt`, such as `请严格按照 3:4 的画面比例生成图片。`.
...
> 请帮我配置并使用同步流式图片生成 API。请先向我索取 Base URL（包含 /v1）和 API Key；收到后仅用于本次会话，不要写入代码、URL、日志或提交到仓库。默认使用 Responses API 的 `/responses`，`stream=true`，在 `image_generation` 工具中使用图片模型并设置 `partial_images=2`；生成或编辑时持续读取流式事件直到最终图片完成，编辑支持多张参考图片。请先验证配置，再开始生图。
